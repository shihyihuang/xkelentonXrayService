...
-- --------------------------------------------------
-- Date Created: 10/06/2023 12:04:47
...
-- Creating foreign key on [PatientId] in table 'Feedbacks'
ALTER TABLE [dbo].[Feedbacks]
ADD CONSTRAINT [FK_FeedbackPatient]
    FOREIGN KEY ([PatientId])
    REFERENCES [dbo].[Patients]
        ([Id])
    ON DELETE NO ACTION ON UPDATE NO ACTION;
...
-- Creating non-clustered index for FOREIGN KEY 'FK_FeedbackPatient'
CREATE INDEX [IX_FK_FeedbackPatient]
ON [dbo].[Feedbacks]
    ([PatientId]);
GO
